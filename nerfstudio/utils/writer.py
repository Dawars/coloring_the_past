...
@check_main_thread
def setup_event_writer(is_wandb_enabled: bool, is_tensorboard_enabled: bool, log_dir: Path, exp_name: str) -> None:
    """Initialization of all event writers specified in config
    ...
    if is_wandb_enabled:
        curr_writer = WandbWriter(log_dir=log_dir, exp_name=exp_name)
        EVENT_WRITERS.append(curr_writer)
...
class WandbWriter(Writer):
    ...
    def __init__(self, log_dir: Path, exp_name: str):
        wandb.init(project="sdfstudio2", name=exp_name, dir=str(log_dir), reinit=True)
