...
class PixelSampler:  # pylint: disable=too-few-public-methods
    ...
    def sample_method(  # pylint: disable=no-self-use
        ...
        if isinstance(mask, torch.Tensor):
            # nonzero_indices = torch.nonzero(mask[..., 0], as_tuple=False)  # todo assumes only nonzero indices are stored
            nonzero_indices = mask
            chosen_indices = random.sample(range(len(nonzero_indices)), k=batch_size)
            indices = nonzero_indices[chosen_indices]

            indices = torch.cat([torch.full((batch_size, 1), -1, device=device), indices], dim=-1) # -1 must be overriden by image_idx
    ...
    def collate_image_dataset_batch(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        if "mask" in batch:  # todo support storing nonzero indices in mask
    ...
    def collate_image_dataset_batch_list(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        image_keys = ['image', 'fg_mask', 'normal_image', 'depth_image', 'sensor_depth', 'is_gray']
        if "mask" in batch:
            ...
            for i in range(num_images):
                ...
                for key in image_keys:
                    if key in batch:
                        all_image_fields[key].append(batch[key][i][indices[:, 1], indices[:, 2]])
    ...
    def sample(self, image_batch: Dict):
        ...
        if isinstance(image_batch["image"], list):
            ...
            for key, value in image_batch.items():
                if isinstance(value, BasicImages):
                    image_batch[key] = value.images

            # remove empty entries from list (no pcd)
            if "sparse_sfm_points" in image_batch:
                image_batch["sparse_sfm_points"] = [item for item in image_batch["sparse_sfm_points"] if item.size()[0]]
